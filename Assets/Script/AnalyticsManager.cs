...
public static class AnalyticsManager
{
    ...
    /// <summary>
    /// 現在の同意状態を UMP（Google User Messaging Platform）から判定して反映する。
    ///
    /// 【なぜ UMP を使うか】
    ///   同意が必要な地域（EEA/UK/スイス）かどうかの判定と同意 UI は、広告用に導入済みの
    ///   UMP が既に行っている。解析用にもう一つ別の同意 UI を作るとユーザーに二重に尋ねる
    ///   ことになるため、UMP の判定結果を単一の同意ゲートとして流用する。
    ///
    /// 【判定】
    ///   - 同意不要な地域（日本など）  → 収集する
    ///   - 同意取得済み                → 収集する
    ///   - 同意が必要だが未取得        → 収集しない
    ///   - ユーザーが設定で明示的にオフ → 収集しない（地域に関わらず優先）
    ///
    /// 呼ぶタイミング: InitializeAsync 完了後と、UMP の同意状態が確定/変更されたとき。
    /// </summary>
