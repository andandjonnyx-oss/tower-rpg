...
public class GameState : MonoBehaviour
{
    ...
    // =========================================================
    /// <summary>
    /// 初期アイテム（薬草・ファイアボール・木刀など）を付与済みかどうか。セーブ対象。
    /// 「セーブ有無」ではなくこのフラグで初期アイテム付与を冪等に管理する。
    ///
    /// false の間だけ EnsureStartingItems() が GrantStartingItems() を実行する。
    /// 一度 true になれば二度と再付与されない（アイテムを使い切っても再付与しない）。
    ///
    /// 既存セーブ（このフィールドを持たない旧データ）はデフォルト false でロードされるが、
    /// SaveManager.Load() 内の移行処理で true に矯正される
    /// （= セーブが存在した既存プレイヤーには再付与しない）。
    /// 初期化（OnReset）で false に戻る。
    /// </summary>
    ...
    /// <summary>
    /// オプション画面から戻る先のシーン名。
    /// Option へ遷移する直前に OpenOptionButton がセットする。
    /// previousSceneName（バトル↔アイテム用）とは独立。競合させない。
    /// </summary>
