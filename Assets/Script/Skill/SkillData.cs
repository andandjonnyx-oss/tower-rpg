...
public class SkillData : ScriptableObject
{
    ...
    [Tooltip("true の場合、命中判定をスキップして必ず命中する（必中技）。\n"
       + "回避バフ・暗闇の影響も受けない。\n"
       + "デバフ系の非ダメージスキル等に設定する。")]
    public bool isGuaranteedHit = false;

    // =========================================================
    // ミス時の追加効果実行（自爆系用）
    // =========================================================
    [Header("Miss Behavior")]
    [Tooltip("true の場合、攻撃が外れても追加効果（SelfDestruct 等）を実行する。\n" +
             "自爆技で外しても自滅させたい場合に ON にする。\n" +
             "デフォルト false（毒の一刺し等、外したら自滅しない技はそのまま）。")]
    public bool executeEffectsOnMiss = false;
