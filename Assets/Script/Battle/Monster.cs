...
public class Monster : ScriptableObject
{
    ...
    [Header("Feed Item")]
    [Tooltip("ONにすると、bossFeedItem=true のアイテムを戦闘中に使用した時に即勝利（イベント勝利）になる。\n"
           + "OFFの場合、餌付けアイテムは通常の回復アイテムとして消費される。")]
    public bool acceptsFeedItem = false;
    ...
    [Tooltip("1ターンの行動回数。デフォルト1。\n"
           + "2以上で複数回行動。各行動は独立に抽選される。\n"
           + "スタン/麻痺は全行動キャンセル。\n"
           + "強制行動(enemyNextForceSkill)は1ターン全消費。\n"
           + "1回目で敵orプレイヤーが倒れたら残りスキップ。")]
    ...
    [Header("Low HP Action Pattern")]
    [Tooltip("HPがこの割合以下になると lowHpActions に切り替わる（0.0〜1.0）。\n"
           + "0 = 切り替えなし。0.3 = HP30%以下で切り替え。")]
    [Range(0f, 1f)]
    public float lowHpThreshold = 0f;

    [Tooltip("低HP時の行動テーブル。threshold の昇順に並べる。\n"
           + "lowHpThreshold > 0 かつ配列が空でない場合に使用。")]
    public EnemyActionEntry[] lowHpActions;

    [Tooltip("低HP時の行動判定の乱数上限基準値。\n"
           + "0の場合は通常の baseActionRange を使用。")]
    public int lowHpBaseActionRange = 0;

    [Tooltip("低HP時の行動回数。\n"
           + "0の場合は通常の actionCount を使用。\n"
           + "瀕死時に2回行動にする場合は2を設定。")]
    public int lowHpActionCount = 0;
