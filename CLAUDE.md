...
- 文言のプラットフォーム差の置き場所（2026-09-18 確定）:
  - **コードが出す文言**（コンティニュー確認）は `ContinueGate.PopupText` の `#if`。
  - **シーン内の固定テキスト**（Tower 倉庫確認・Status の SP 振り直し確認）は
    `Assets/Scenes/Console/` 側のシーンを直接書き換える（対応済み）。シーンが二本立て
    なのでシンボル分岐は不要。モバイル側の文言を変えても Console 側には波及しない点に注意。
  - **会話**は `TalkEvent.consoleLines`。空なら `lines`、中身があれば CONSOLE_BUILD 時のみ
    そちらを使う。⚠️ 台詞を読むコードは `lines` を直接参照せず、必ず
    `TalkEvent.ActiveLines` を使うこと（直接読むとコンソール版でスマホ前提の台詞が出る）。
    現状の読み手は `TalkRunner` のみ（図鑑リプレイも同経路）。1行だけの変更でも会話全体を
    `consoleLines` に複製する（まるごと差し替えと同じ仕組みで扱うため）。
    図鑑タイトルも同じ考え方で `TalkEvent.consoleZukanTitle`（空なら `zukanTitle`）。
    読み手は `TalkEvent.ActiveZukanTitle` を使う（id フォールバックもここに集約）。
    ⚠️ **コンソール用に別アセット（別 id）を作って差し替えないこと。** id が分かれると
    既読管理・図鑑の一覧・`exclusiveIds`・出現条件までプラットフォーム別に面倒を見る
    必要が出る。差はフィールドで持ち、id は共有する。
    要対応の会話（キーワード走査で検出）: F31_S04（レビュー依頼・入力済み）/
    F37_S02・F55_S02（広告の話題。台詞とタイトルの入力は未了）。
- コンソール版の残課題: 上記ポップアップの「はい」ボタン等のラベル確認。

## 11. コンソール版シーンは「同名コピー＋シーンリスト切り替え」方式

コンソール版はタッチ前提の分散配置と操作感が合わないため、レイアウトを作り替えた
シーンを別ファイルで持つ（2026-09-01 決定）。**プロジェクトは分けない。**
シーンだけ二本立てにし、コード・アセット・台詞は全プラットフォームで共有する。

- **置き場所と命名**: コンソール版シーンは `Assets/Scenes/Console/` に
  **モバイル版と同じファイル名**で置く（例: `Console/Battle.unity`）。
  `SceneManager.LoadScene("Battle")` はビルド設定リスト内の名前で解決されるため、
  **遷移コードは一切変えない**。`GetActiveScene().name` も共通のまま。
- **切り替え**: メニュー `Tools > コンソール版シーン` でシーンリストを一括切り替え。
  コンソール版が存在するシーンだけ差し替わり、無いシーンは共有のまま。
  エディタでコンソール版を再生テストする時もこれで切り替える。
- ⚠️ **モバイル向けビルド前は必ずモバイル版リストへ戻す。**
  2026-09-18 から、このメニューは **CONSOLE_BUILD シンボルも連動して付け外しする**
  （対象は現在アクティブなビルドターゲット。戻す時は Android / iOS からも必ず外す）。
  エディタのコンパイルはアクティブターゲットのシンボルに従うので、ターゲットが Android の
  ままでもコンソール仕様（広告なし・残数制コンティニュー・consoleLines）を再生テストできる。
  戻し忘れの保険として `ConsoleBuildGuard`（同ファイル）が、Android / iOS ビルド開始時に
  シンボルか Console 版シーンが残っていればビルドを失敗させる。
- ⚠️ **シーンをコピーするときは `.unity` ファイルだけをコピーし、`.meta` は
  コピーしないこと。** meta ごとコピーすると GUID が重複してプロジェクトが壊れる。
  Unity 上の Ctrl+D 複製なら安全（新 GUID が自動で振られる）。
- ⚠️ **二重メンテの掟**: 共有ロジックに関わるシーン修正（ボタンの配線・
  コンポーネント参照・オブジェクトの追加削除）を `Assets/Scenes/` 側に入れたら、
  同名の Console/ 版があるか確認し、あれば**同じ修正を両方に入れる**。
  レイアウト（RectTransform）だけの変更は各版で独立してよい。
- 現状の Console/ 版: **ビルド対象の全20シーン**（2026-09-14 に全シーン複製へ方針変更。
  当初は「最小限に保つ」方針だったが、コンソール版はレイアウトを広く作り替えるため
  全複製とした）。
  ⚠️ その結果、**二重メンテの掟は全シーンに適用される**。モバイル側のシーンに
  オブジェクト追加・ボタン配線・コンポーネント参照の変更を入れたら、必ず
  Console/ 側の同名シーンにも同じ変更を入れること。
