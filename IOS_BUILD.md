...
4. **`bundleVersion`（表示バージョン）の番号規約 ⚠️** iOS はバージョンを
   「ピリオド区切りの整数列」として比較する（`01` は `1`、`20` は `20`）。
   採番の経緯: 1.0 → 1.01 (2026-08-30) → **1.2.0 (2026-09-08)**。
   1.2.0 以降は **3要素のセマンティックバージョン（major.minor.patch）で採番する**
   （1.2.0 → 1.2.1 → 1.3.0 → … → 2.0.0）。1.01 は Apple には (1,1) と読まれる
   ため、1.2.0 = (1,2,0) は正しく「上がった」と判定される。
   - ❌ 第2要素を 2 桁連番（1.20 等）にする旧案は採用しない。1.20 の後に
     1.2.1 や 1.3.0 へ進めなくなる（3 < 20 で「下がった」と弾かれる）ため。
   - ❌ 各要素の先頭に 0 を付けない（`1.01` のような表記は Apple には 1.1 と
     読まれ、表示と比較結果が食い違う）。
   - Google Play は versionName の大小を見ない（versionCode だけ）ため、
     この制約は iOS 側だけの話。ただし表示バージョンは Android と共通の
     1フィールドなので、両ストアとも同じ番号で揃える。
   - ビルド番号は Android の versionCode と iOS の `buildNumber.iPhone` を
     同じ値で揃える（1.2.0 は両方 7）。
5. Unity でビルド → 第4-1節の Archive / 書き出し → 下記の検証 → Transporter でアップロード。
