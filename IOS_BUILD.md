...
4. **`bundleVersion`（表示バージョン）の番号規約 ⚠️** iOS はバージョンを
   ...
   本作は 1.0 → 1.01 (2026-08-30) → 次回 **1.20** と採番した経緯があり、
   1.20 以降は**第2要素を2桁の連番として使い続けること**
   （1.20 → 1.21 → … → 1.99 → 2.00）。
   - ❌ `1.20 → 1.3` は 3 < 20 で「バージョンが下がった」と判定され弾かれる
   - ❌ `1.20 → 1.2.1` も同じ理由でアウト
   - Google Play は versionName の大小を見ない（versionCode だけ）ため、
     この制約は iOS 側だけの話。ただし表示バージョンは Android と共通の
     1フィールドなので、両ストアともこの連番で揃える。
