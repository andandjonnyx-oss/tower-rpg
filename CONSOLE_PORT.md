...
| 画面 | 主な内容 | commit |
|---|---|---|
| 戦闘 | 十字キーは右6コマンドの縦ループのみ（押せるボタンだけ毎F張り直し）。**既定フォーカス＝武器スキル、CT中は攻撃**（自ターン毎に戻る＝決定連打で繰り返せる）。G=ギブアップ / O=ログ拡大（キー専用）/ 拡大中↑↓=ページ送り。ポップアップにモーダルスコープ | `21ba217` `f3c9fe4` `3cde7e8` |
| 塔内部 | 右側コマンドを位置順の縦ループ（押せるボタンだけ毎F張り直し。非表示の倉庫/魔法は候補に残す）。**既定フォーカス＝進む**（決定連打で進める）。帰還/倉庫確認・アイテム拾得・魔法一覧はモーダルスコープ（拾得の初期フォーカス＝入手する、Esc/B＝諦める）。麻痺/石化ブロッカーはキャンセル不可スコープでフォーカスを空にする | （未コミット） |
| 戦闘中アイテム | **2026-09-18 に通常時の Itembox と同方式へ統一**（格子⇔詳細ボタン⇔戻る、決定で押す、Esc/Bで詳細閉→戦闘へ）。旧方式（格子固定＋1/2キー・パッドX/Y）は廃止。戦闘中の差は GetButtons が出すボタンの数と戻り先だけ | `a7cecaf` ほか（統一は未コミット） |
...
9. 既定フォーカスは `SelectionHighlighter.PreferredFallback` を毎フレーム更新して指定する
   （Tower＝進む、Battle＝スキル/攻撃）。選択が外れた瞬間に自動で既定へ戻るので、
   決定ボタン連打の操作感が成立する。ポップアップ側は `ModalFocusScope.preferred`。
   ⚠️ シーン到着直後は常駐の SelectionHighlighter.Update がシーン側 Update より先に
   走ることがあり、既定が未設定のまま「左上のボタン」が選ばれて固定される。
   既定ボタンが押せるようになった最初のフレームで `SelectionHighlighter.SelectNow()` を
   一度呼んで上書きすること（Tower/Battle は `defaultFocusApplied` で実施）。
   スコープ配下に押せるものが無い間（入力ロック・ブロッカー）はスコープ外の選択を
   空にする（連打が裏のボタンへ抜ける事故の防止）。
...
- 選択枠の見た目調整（`SelectionHighlighter` 冒頭の定数）。2026-09-17 に方式を確定:
  羊皮紙ボタン（`youhisi`/`yousihi`/`consumeyou`）＝黄色(255,255,0)着色、
  それ以外＝赤枠、Tower/Battle/Main/Zukan/Title＝ちびキャラを左右反転してボタン左に配置
  （スプライトは `Resources/SelectionHighlighterConfig.asset` 経由。画像は Art 側に据え置き）。
  実機での見た目（サイズ・隙間）は未確認。定数 `CursorGap` / `CursorHeightFactor` で調整。
